...
#       urls_list.txt would mark everything deleted and poison the next run)
...
result <- tryCatch({
  ...
  # Plausibility guard: a truncated or empty listing must not poison the
  # cache. The dataset has only ever grown; a >10% shrink is a fetch
  # problem until proven otherwise.
  if (length(cached_set) > 0 && length(fresh_set) < 0.9 * length(cached_set)) {
    stop(sprintf(
      "fresh listing (%d URLs) is < 90%% of cached (%d) - refusing to update cache (truncated fetch?)",
      length(fresh_set), length(cached_set)
    ))
  }
...
if (identical(result, "error")) {
  cat("Error during change detection - exit code 2\n")
  quit(status = 2)
