...
test_only = False
test_number_items = 10  # Only used when test_only=True

# Incremental mode - set to True to process only new URLs from data/urls_new.txt
incremental = False  # Set to True for weekly updates after initial catalog build
...
# Load list of image paths (from urls_list.txt or urls_new.txt for incremental)
urls_file = "data/urls_new.txt" if incremental else "data/urls_list.txt"
...
print(f"Processing {len(urls_to_check)} URLs (test_only={test_only}, incremental={incremental})")
...
# Determine which URLs need validation
if incremental:
    # In incremental mode, only validate new URLs not in cache
    urls_to_validate = [url for url in urls_to_check if url not in existing_urls]
    print(f"Incremental mode: {len(urls_to_validate)} new URLs need validation")
else:
    # In full mode, validate all URLs in the list
    urls_to_validate = urls_to_check
...
print(f"Creating STAC items for {len(urls_to_check)} URLs...")
try:
    with concurrent.futures.ThreadPoolExecutor() as executor:
...
print(f"\nValidating {len(list(collection.get_items()))} items...")
